 ...
 * to the dark-theme defaults when a token is missing — keeps components
 * usable when @altara/core's stylesheet hasn't been imported yet.
 */
export interface AeroTokens {
  bgPanel: string;
  bgElevated: string;
  border: string;
  textPrimary: string;
...
export function readTokens(el: HTMLElement | null): AeroTokens {
